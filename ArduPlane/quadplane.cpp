...
bool QuadPlane::setup(void)
{
    ...
    if (hal.util->available_memory() <
        4096 + sizeof(*motors) + sizeof(*attitude_control) + sizeof(*pos_control) + sizeof(*wp_nav) + sizeof(*ahrs_view) + sizeof(*loiter_nav) + sizeof(*weathervane)) { //structurelog
    ...
    if (motors->armed()) {
        ...
        if (motors_active && (motors->get_spool_state() != AP_Motors::SpoolState::SHUT_DOWN)) {
            ...
            if (now - last_ctrl_log_ms > 100) {
                ...
                motors->Log_Write();
                //motors->Log_WriteCu(); //structurelog
            }
        }
